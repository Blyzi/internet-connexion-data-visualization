...
from utils import compute_pdk_column_map, list_elig, list_techno, elig_color, techno_color, department_list, color_to_hex, color_to_alt, compute_folium_map, load_data_commune
...
df = df[df['Department Code'] == dep_select]
...
with col1:
    ...
    st.metric("Number of commune", df['INSEE Code'].nunique())
...
st.plotly_chart(px.imshow(df_toggle.drop(['Commune Name', 'INSEE Code', 'Department Code', 'Region Code', 'geometry', 'Entity Number'], axis=1).corr(
...
"""
---
Map of the region by cumulative internet eligibility ratio
"""
...
m = compute_folium_map(df_map, 'Region Code', 'ratio',
                       f'Ratio of eligible entity to {option_elig}', bins=[0, 0.25, 0.5, 0.75, 1], zoom_start=8)
...
st.altair_chart(alt.Chart(df.drop(['geometry', 'Region Code', 'Department Code', 'Commune Name', 'INSEE Code'], axis=1).groupby(['Year']).sum().reset_index().melt(id_vars=['Year'], value_vars=list_techno, var_name='Technology', value_name='Sum')).mark_line().encode(
    x='Year:O',
...
m, legend = compute_pdk_column_map(
    df_map, list_techno, radius=500, elevation_scale=0.1, color=techno_color, zoom=8,
    tooltip={'text': "Commune Name: {Commune Name} ({INSEE Code})\n Eligible entity: {Entity Number} \n" + '\n'.join([f"{tech}: {{{tech}}}" for tech in list_techno])})
